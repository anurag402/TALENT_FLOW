...
export default function AssessmentsPage() {
      ...
      for (const job of jobsData.jobs || []) {
        try {
          ...
          const assessmentResponse = await fetch(
            `http://backend/assessments/${job.id}`
          );
          const assessmentData = await assessmentResponse.json();
  ...
  const jobsWithAssessments = filteredJobs.filter(
    (job) => (assessments[String(job.id)] || []).length > 0
  );
  const jobsWithoutAssessments = filteredJobs.filter(
    (job) => (assessments[String(job.id)] || []).length === 0
  );
      ...
      <div className="container mx-auto p-6 max-w-7xl">
        <div ref={heroRef} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 rounded-full bg-white/20 shadow-md">
              <ClipboardList className="h-8 w-8 text-white" />
            </div>
            <h1 className="text-3xl font-extrabold text-white">Assessments</h1>
          </div>
          <p className="text-white/80 max-w-2xl">
            Create, preview and manage job-specific assessments.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 items-center">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-full sm:w-[180px] bg-white/80 backdrop-blur-sm border-0 shadow-lg">
          ...
          <Button
            onClick={() => fetchJobsAndAssessments()}
            variant="ghost"
          ...
          >
            <svg
              ...
              strokeWidth="2"
            >
              <path d="M21 12a9 9 0 1 1-3-6.72" />
              <path d="M21 3v6h-6" />
            </svg>
            Refresh
          </Button>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-white/80">Loading assessments...</div>
          </div>
        ) : (
          <>
            <h2 className="text-xl font-semibold text-white/90 border-b-2 border-white/20 pb-2 mb-4">
              Jobs with Assessments{" "}
              <span className="text-sm text-white/70">
                ({jobsWithAssessments.length})
              </span>
            </h2>
                            ...
                            <div className="text-sm text-gray-600">
                              {(() => {
                                const jobAssessments =
                                  assessments[String(job.id)] || [];
                                const totalQuestions = jobAssessments.reduce(
                                  (sum, a) => sum + (a.questions?.length || 0),
                                  0
                                );
                                if (jobAssessments.length === 0)
                                  return "0 questions";
                                if (jobAssessments.length === 1)
                                  return `${totalQuestions} questions`;
                                return `${jobAssessments.length} assessments • ${totalQuestions} questions`;
                              })()}
                            </div>
                            <div className="space-y-2">
                              {(assessments[String(job.id)] || []).map(
                                (assessment) => (
                                  <Card
                                    key={assessment.id}
                                    className="bg-white/90 border border-gray-200 shadow-sm p-3"
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex gap-2">
                                        ...
                                        >
                                          <Link
                                            to={`/assessments/preview/${job.id}/${assessment.id}`}
                                          >
                                        ...
                                        >
                                          <Link
                                            to={`/assessments/edit/${job.id}/${assessment.id}`}
                                          >
                      ...
                      <Card className="hover:shadow-lg transition-shadow border-dashed border-2 border-white/30 bg-white/50 backdrop-blur-sm">
                        ...
                        <CardContent>
                          <motion.div whileHover={{ scale: 1.02 }}>
                            <Button
                              asChild
                              className="w-full bg-gradient-to-r from-[#14B8A6] to-[#3B82F6] text-white hover:from-[#14B8A6]/90"
                            >
                              <Link to={`/assessments/new/${job.id}`}>
            ...
            {filteredJobs.length === 0 && (
              <div className="text-center py-12">
                <ClipboardList className="h-12 w-12 text-white/50 mx-auto mb-4" />
