      ...
      routes() {
        ...
        this.get("/jobs", (schema, req) => {
          try {
            ...
            return new Response(
              500,
              {},
              { error: `Something went wrong in fetching jobs: ${e}` }
            );
          }
        });
        this.post("/jobs", (schema, req) => {
          try {
            ...
            const missing = requiredFields.filter(
              (f) => attrs[f] === undefined || attrs[f] === null
            );
        ...
        this.patch("/jobs/:id", (schema, req) => {
          try {
            ...
            if (
              updateAttrs.status &&
              !["active", "archived"].includes(updateAttrs.status)
            ) {
              return new Response(400, {}, { error: "Invalid status value" });
            }
            ...
            if (
              updateAttrs.tags &&
              (!Array.isArray(updateAttrs.tags) ||
                updateAttrs.tags.length === 0)
            ) {
              return new Response(
                400,
                {},
            ...
            if (
              updateAttrs.applicants &&
              (typeof updateAttrs.applicants !== "number" ||
                updateAttrs.applicants < 0)
            ) {
              return new Response(
                400,
                {},
        ...
        this.patch("/jobs/:id/reorder", (schema, request) => {
          ...
          const originalOrder = [...allJobs].sort(
            (a, b) => a.orderId - b.orderId
          );

          // Store original orderIds for rollback
          const originalOrderIds = originalOrder.map((job) => ({
            id: job.id,
            orderId: job.orderId,
          }));
          ...
          if (Math.random() < 0.1) {
            // Rollback by restoring original orderIds
            originalOrderIds.forEach(({ id, orderId }) => {
              const job = schema.jobs.find(id);
              if (job) {
                job.update({ orderId });
              }
            ...
            return new Response(
              500,
              {},
              { error: "Reorder failed, rolled back" }
            );
          }
          ...
          return {
            success: true,
            jobs: newOrder.map((job) => ({ id: job.id, orderId: job.orderId })),
          };
        });
        this.get("/candidates", (schema, req) => {
          try {
            ...
            return new Response(
              500,
              {},
              { error: `Something went wrong in fetching candidates: ${e}` }
            );
          }
        });
        ...
        this.get("candidates/:id/timeline", (schema, req) => {
          try {
            ...
            return new Response(200, {}, { timeline });
          } catch (e) {
            return new Response(
              500,
              {},
        ...
        this.get("/assessments/:param", (schema, req) => {
          try {
            ...
            const assessments =
              schema.assessments.where({ jobId: param }).models || [];
            return new Response(200, {}, { assessments });
          } catch (e) {
            return new Response(
              500,
              {},
              { error: `Something went wrong in fetching assessments: ${e}` }
            );
          }
        });
          ...
          this.post("/assessments/create", (schema, req) => {
            try {
              ...
              if (
                !attrs.title ||
                !attrs.jobId ||
                !Array.isArray(attrs.questions)
              ) {
                return new Response(
                  400,
                  {},
                  {
                    error: "Missing required fields: title, jobId, questions[]",
                  }
                );
              }
